...
const Detail = ({ onAdd }) => {
  ...
  const image = "https://backend-nodejs-61ox.onrender.com/";
  useEffect(() => {
    const initData = async () => {
      try {
        setIsLoading(true);
        // const response = await getListProduct(
        //   `https://backend-nodejs-61ox.onrender.com/product/${productId}`
        // );
        const response = await callAPI(
          `https://backend-nodejs-61ox.onrender.com/product/${productId}`
        );
        const response1 = await callAPI(
          `https://backend-nodejs-61ox.onrender.com/review/${productId}`
        );
